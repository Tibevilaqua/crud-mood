(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('crudMoodApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-crudMoodApp-error');
                var entityKey = httpResponse.headers('X-crudMoodApp-params');
                if (errorHeader) {
